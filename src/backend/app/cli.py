...
def simulate(
    fresh: bool = typer.Option(
        True,
        "--fresh/--keep",
        help="Reset the event to DRAFT before running (default), or keep its current state.",
    ),
) -> None:
    """
    Run the Rite of Many Hands — a full lifecycle simulation.

    Summons a cast of human and machine agents and drives the event through
    every state (DRAFT → OPEN → FROZEN → FINAL → ARCHIVED) over the real API:
    the gathered register, teams form, the gates close, a verdict is sealed,
    and the chronicle is read back.

    A test tool and a living demo. With ``--fresh`` (default) the event is
    reset to DRAFT first; ``--keep`` runs against the current state.

    Examples::

        hackritual simulate
        hackritual simulate --keep
    """
    import asyncio
    ...
    def narrate(kind: str, message: str) -> None:
        console.print(message, style=_STYLE.get(kind, "white"))

    console.print("[bold magenta]◆ The Rite of Many Hands[/]\n")
    try:
        report = asyncio.run(run_ritual(narrator=narrate, fresh=fresh))
    except Exception as exc:
        err_console.print(f"The ritual collapsed: {exc}")
        raise typer.Exit(code=1)

    table = Table(title="The ritual is complete", show_header=True)
    table.add_column("Measure", style="bold")
    table.add_column("Value")
    table.add_row("final state", f"[green]{report.final_state}[/]")
    table.add_row("states visited", " → ".join(report.states_visited))
    table.add_row("participants", str(report.participants_created))
    table.add_row("teams formed", str(report.teams_created))
    table.add_row("members joined", str(report.members_joined))
    table.add_row("projects proposed", str(report.projects_proposed))
    table.add_row("submissions", str(report.submissions_created))
    table.add_row("agent submissions", str(report.agent_submissions))
    table.add_row("files attached", str(report.files_attached))
    table.add_row("leaderboard entries", str(report.leaderboard_entries))
...
# --------------------------------------------------------------------------- #
# Entry point for `python -m app.cli` (in addition to the installed `hackritual`)
# --------------------------------------------------------------------------- #
