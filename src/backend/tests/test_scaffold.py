...
async def test_get_doc_path_traversal_blocked(client):
    ...
    # FastAPI URL routing collapses the slashes; the endpoint sees only the last
    # segment (a non-.md file) → blocked.
    assert r.status_code in (400, 404)
    # Test the explicit check via a query-encoded traversal attempt.
