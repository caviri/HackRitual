...
export default function JudgingPage() {
  ...
  // Server-authoritative scoring roll-up (admin-only). null in demo mode.
  const [status, setStatus] = useState<ScoringStatusDTO | null>(null);
  ...
  async function loadAll() {
    await load();
    setStatus(await api.adminScoringStatus());
  }
  ...
  return (
    <>
      ...
      <section className="mx-auto w-full max-w-5xl px-6 py-10 space-y-6">
        {status && <ScoringSummary status={status} />}
        {error && (
        ...
        {rows === null ? (
          ...
          rows.map((row) => (
            <JudgingRow key={row.submission.id} row={row} onChanged={loadAll} />
          ))
        )}
      </section>
    </>
...
/** Server-authoritative scoring roll-up — scorer identity, status counts, and a
 * value histogram, from `/api/admin/scoring/status`. */
function ScoringSummary({ status }: { status: ScoringStatusDTO }) {
  const maxBucket = Math.max(1, ...status.distribution.map((d) => d.count));
  const statuses = Object.entries(status.counts_by_status);
  return (
    <article className="ascii-frame p-5">
      <div className="flex flex-wrap items-baseline justify-between gap-3 mb-4">
        ...
        <p className="font-mono text-[0.7rem] text-fg-muted">
          ...
          <span className="text-warm">
            {status.scorer.type} · {status.scorer.version}
          </span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <div className="border border-rule px-3 py-2">
          ...
          <p className="font-display italic text-2xl leading-none text-primary tabular-nums">
            {status.scored_total}
          </p>
        </div>
        <div className="border border-rule px-3 py-2">
          ...
          <p className="font-display italic text-2xl leading-none text-accent tabular-nums">
            {status.average_score != null ? status.average_score.toFixed(1) : '—'}
          </p>
        </div>
        {statuses
          .filter(([s]) => s !== 'scored')
          .slice(0, 2)
          .map(([s, n]) => (
            <div key={s} className="border border-rule px-3 py-2">
              <p className="font-mono text-[0.62rem] uppercase tracking-widest text-fg-dim mb-1">
                {s}
              </p>
              <p className="font-display italic text-2xl leading-none text-fg-muted tabular-nums">
                {n}
              </p>
            </div>
          ))}
      ...
      <p className="font-mono text-[0.62rem] uppercase tracking-widest text-fg-dim mb-2">
        distribution
      </p>
      <div className="space-y-1.5">
        {status.distribution.map((d) => (
          <div key={d.range} className="flex items-center gap-2 font-mono text-[0.7rem]">
            <span className="text-fg-dim w-12 tabular-nums shrink-0">{d.range}</span>
            <span
              className="bg-primary/30 border-r-2 border-primary h-3 inline-block"
              style={{ width: `${(d.count / maxBucket) * 100}%` }}
              aria-hidden
            />
            <span className="text-fg-muted tabular-nums">{d.count}</span>
          </div>
        ))}
      </div>
    </article>
