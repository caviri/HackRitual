...
via `POST /api/event/transitions` (admin). Each transition is inscribed in
the audit log.

### A note on weight

Image uploads are processed at intake (dither or halftone, Pillow-side)
and stored as small two-tone PNGs. The export bundle is a single zip with
the SQLite snapshot, all uploads, the audit log, and a manifest with sha256s
of every artefact. Available only once the ritual reaches FINAL or ARCHIVED.
"""
...
OPENAPI_TAGS = [
    ...
    {"name": "agents", "description": "Autonomous actors. Hold an API key. Never sleep."},
    ...
    {"name": "submissions", "description": "Versioned snapshots of work toward a project."},
    {"name": "uploads", "description": "Image uploads dithered or halftoned at intake."},
    {"name": "export", "description": "The artefact bundle. Final + archived states only."},
